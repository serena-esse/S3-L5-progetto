...
let biggerNumber = whoIsBigger(5, 7);
...
let Array = splitMe("I love coding");
console.log(Array);
...
*/
/*let regex_email_valida =
  /^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
function isThisAnEmail(str) {
  return regex_email_valida.test(str);
}
...
//ho ricopiato la funzione precedente
/*function dice() {
  return Math.floor(Math.random() * 6);
}
...
console.log("Sum of dices rolls:", result);
...
const myBirthday = new Date("1990-07-21");
...
const millenniumMovies = onlyInLastMillennium(movies);
...
const totalYearsOfMovies = sumAllTheYears(movies);
console.log(totalYearsOfMovies);
...
const str = "Rings";
const foundMovies = searchByTitle(str, movies);
...
/*function searchAndDivide(str, movies) {
  ...
  const match = movies.filter((movie) => movie.Title.toLowerCase().includes(lowerCaseStr));

  const unmatch = movies.filter((movie) => !movie.Title.toLowerCase().includes(lowerCaseStr));

  return { match, unmatch };
}
const result = searchAndDivide("Rings", movies);
...
/*function removeIndex(index, movies) {
  ...
  if (index >= 0 && index < moviesCopy.length) {
    ...
    console.log("Indice non valido. Nessun elemento rimosso.");
  }

  return moviesCopy;
}
const indexToRemove = 4;
const moviesWithoutRemovedIndex = removeIndex(indexToRemove, movies);

console.log("Array", moviesWithoutRemovedIndex);
